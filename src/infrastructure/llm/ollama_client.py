import logging
import re
from typing import AsyncIterator

from openai import AsyncOpenAI

logger = logging.getLogger(__name__)

SEARCH_SIGNAL = "[SEARCH]"
TEMPLATE_SIGNAL_PREFIX = "[TEMPLATE:"

CLASSIFY_SYSTEM_PROMPT = """Ти — AI-помічник Управління Державної охорони України (УДО).

ГОЛОВНЕ ПРАВИЛО — ОБОВ'ЯЗКОВО визнач тип питання ПЕРЕД відповіддю:

1. ШАБЛОНИ ДОКУМЕНТІВ.
[TEMPLATE:id] — ТІЛЬКИ коли користувач ЯВНО просить саме ФАЙЛ: "дай шаблон", "дай бланк", "надішли рапорт", "скачати форму", "хочу зразок рапорта (файл)".
Якщо користувач питає ПРО тему (отримати допомогу, як оформити, що таке, розкажи про) — завжди [SEARCH]. Спочатку відповідь з документів, потім можна запропонувати шаблон файлом.

[SEARCH]: "Я хочу отримати матеріальну допомогу", "розкажи про мат допомогу", "як оформити відпустку", "як піти у відпустку", "що потрібно для звільнення".
[TEMPLATE]: "Дай шаблон рапорта на мат допомогу", "надішли бланк на відпустку", "скачати форму пояснювальної".

ДОСТУПНІ ШАБЛОНИ: відпустка, звільнення, мат_допомога, пояснювальна.
Токени: [TEMPLATE:відпустка], [TEMPLATE:звільнення], [TEMPLATE:мат_допомога], [TEMPLATE:пояснювальна].

2. Відповідай ТІЛЬКИ токеном [SEARCH] якщо питання хоч якось стосується:
- Службових процесів: відпустка, рапорт, звільнення, переведення, відрядження, грошове забезпечення, матеріальна допомога
- IT та обладнання: VPN, пароль, логін, доступ, обліковий запис, пошта, принтер, комп'ютер, ноутбук, мережа, wifi, флешка, USB
- Побуту та режиму: кухня, кавомашина, кава, холодильник, перепустка, їдальня, дрес-код, робоче місце
- Документів та правил: наказ, регламент, інструкція, положення, порядок, статут
- Будь-чого, що МОЖЕ бути специфічним для служби в УДО

ВАЖЛИВО: Якщо ти НЕ ВПЕВНЕНИЙ — завжди відповідай [SEARCH]. Краще зайвий раз пошукати, ніж вигадати відповідь.
ЗАБОРОНЕНО вигадувати службові процедури, контакти чи інструкції.

3. Відповідай одразу (БЕЗ [SEARCH]) ТІЛЬКИ якщо це:
- Привітання, подяка, прощання, casual розмова (як справи, як ти)
- ЕМОЦІЇ ТА РЕАКЦІЇ: смех (ахах, хаха, ахахах, лол, 😂), здивування (ого, вау), емоційні вигуки. На смех відповідай дружньо, наприклад "Радий, що вам сподобалось!" або "😊". НЕ шукай в базі — це не питання!
- ПІДТВЕРДЖЕННЯ: "ок", "зрозумів", "дякую", "ага", "добре" — коротко підтверди ("Будь ласка!", "Звертайтесь!"). НЕ шукай.
- ОДНОСЛОЖНІ ВІДПОВІДІ: "так", "ні", "угу" — якщо це відповідь на твоє попереднє питання, продовжуй діалог. Якщо незрозуміло — уточни.
- Безглузді або випадкові символи (asdf, 123, тощо) — ввічливо уточни що мав на увазі користувач
- Питання про дату, час, день тижня, погоду — відповідай що не маєш такої інформації
- Чисто загальні знання БЕЗ зв'язку зі службою (наука, математика, історія, програмування)

4. Якщо користувач каже "не зрозумів", "можна детальніше?", "поясни інакше" — використай [SEARCH] щоб знайти додаткову інформацію.

МОВА: ТІЛЬКИ українська! ЗАБОРОНЕНО російську (отдел, работа, отпуск). Якщо не знаєш слово — пропусти його.
Формат: коротко і по суті."""

SYSTEM_PROMPT = """Ти — AI-помічник Управління Державної охорони України (УДО). Ти допомагаєш військовослужбовцям та працівникам УДО з питаннями служби, документів і внутрішніх процедур.

Мова:
- Відповідай ВИКЛЮЧНО українською. ЗАБОРОНЕНО російські слова (отдел→відділ, работа→робота, отпуск→відпустка, нужно→потрібно).

Діалог:
- Відповідай на поточне питання. Якщо воно пов'язане з попереднім — використовуй історію.
- На привітання/подяку/прощання відповідай коротко.

Точність:
- Якщо є контекст із документів — відповідай СТРОГО на його основі. Передавай інформацію точно, як у документі.
- ЗАБОРОНЕНО вигадувати будь-що, що стосується служби: процедури, контакти, email, кроки, правила, розташування, структури документів. Якщо цього немає в контексті — скажи що немає і більше нічого не додавай.
- Загальні знання можна використовувати ТІЛЬКИ для питань, які НЕ стосуються служби (наука, математика, програмування тощо).

Формат:
- Факт → 1-3 речення.
- Інструкція → покроково.
- Розмова → коротко."""

SYSTEM_PROMPT_GENERAL = """Ти — AI-помічник.

Мова:
- Відповідай ВИКЛЮЧНО українською. ЗАБОРОНЕНО російські слова (отдел→відділ, работа→робота, отпуск→відпустка).

Діалог:
- Відповідай на поточне питання. Якщо воно пов'язане з попереднім — використовуй історію. Якщо ні — не згадуй минуле.
- На привітання/подяку/прощання відповідай коротко.

Точність:
- Відповідай із загальних знань. Якщо не знаєш — скажи прямо.
- Не згадуй документи чи службові регламенти."""

MODE_DOCS_PROMPT = """РЕЖИМ: ДОКУМЕНТИ.
- Усі відповіді базуй ВИКЛЮЧНО на фрагментах документів нижче.
- ЗАБОРОНЕНО додавати будь-що від себе: факти, кроки, поради, пропозиції, структури, формати.
- Якщо відповіді немає у фрагментах — скажи ТІЛЬКИ що в документах немає такої інформації. НЕ пропонуй альтернатив і НЕ вигадуй.
- Якщо користувач питає про джерело — скажи, що відповідь узята з документів УДО."""

MODE_GENERAL_PROMPT = """РЕЖИМ: ЗАГАЛЬНІ ЗНАННЯ.
- Можна відповідати з загальних знань.
- Якщо не впевнений — скажи, що не знаєш, і не вигадуй.
- Не згадуй документи чи базу знань. Не використовуй фрази на кшталт "у документах".
- Якщо користувач питає про джерело — скажи, що це загальні знання, а не службові документи."""

SEARCH_QUERY_REWRITE_PROMPT = """Ти — генератор пошукових запитів для RAG.
Переформулюй останнє питання користувача в самодостатній запит для пошуку по документах.

Правила:
- Поверни лише текст запиту без пояснень.
- Якщо питання вже самодостатнє, поверни його майже без змін.
- Якщо це уточнення (наприклад "а скільки днів?"), додай потрібний контекст з історії.
- Не вигадуй нових фактів, назв або термінів, яких не було у діалозі.

Приклад:
Історія: [user: Як оформити відпустку?] [assistant: ...]
Питання: А скільки днів дають?
Запит: Скільки днів відпустки надається військовослужбовцю?
"""

PROMPT_WITH_CONTEXT = """Нижче наведено фрагменти з документів УДО.

{context}

---
Питання: {question}

СУВОРІ правила:
- Єдине джерело фактів — ТІЛЬКИ фрагменти вище. Нічого більше.
- ЗАБОРОНЕНО додавати від себе: факти, кроки, поради, пропозиції, "можна спробувати", "рекомендується", структури документів.
- Відповідай ТІЛЬКИ тим, що є у фрагментах. Не плутай схожі теми (відпустка ≠ відрядження).
- Зберігай факти, цифри, терміни точно як у документах.
- Якщо у фрагментах немає відповіді або є лише часткова інформація — передай те що є і скажи що більше інформації в документах немає. Більше нічого не додавай.
- Якщо фрагменти взагалі не стосуються питання — скажи: "В документах УДО немає інформації з цього питання." Більше нічого не додавай.
- ЗАБОРОНЕНО виводити текст шаблонів (рапорт, пояснювальна тощо) — користувач отримує шаблон лише файлом. Можна лише коротко згадати, що є відповідний шаблон (файл), який можна отримати. Не цитуй і не наводь зміст шаблону."""


class OllamaClient:
    """LLM client for Ollama (OpenAI-compatible API)."""

    def __init__(
        self,
        base_url: str = "http://localhost:11434/v1",
        model: str = "qwen2.5:7b",
        max_tokens: int = 1024,
        temperature: float = 0.1,
    ):
        self._client = AsyncOpenAI(base_url=base_url, api_key="ollama")
        self._model = model
        self._max_tokens = max_tokens
        self._temperature = temperature

    async def classify_and_stream(
        self,
        user_message: str,
        history: list[dict] | None = None,
    ) -> AsyncIterator[str]:
        """Classify query via LLM and stream response.

        Yields:
            SEARCH_SIGNAL, "[TEMPLATE:id]", or answer tokens.
        """
        messages: list[dict] = [
            {"role": "system", "content": CLASSIFY_SYSTEM_PROMPT},
        ]

        if history:
            messages.extend(history[-4:])

        messages.append({"role": "user", "content": user_message})

        response = await self._client.chat.completions.create(
            model=self._model,
            messages=messages,
            max_tokens=self._max_tokens,
            temperature=self._temperature,
            stream=True,
        )

        buffer = ""
        classified = False
        max_signal_len = len(SEARCH_SIGNAL) + 25  # room for [TEMPLATE:мат_допомога]

        try:
            async for chunk in response:
                token = chunk.choices[0].delta.content
                if not token:
                    continue

                if not classified:
                    buffer += token
                    stripped = buffer.strip()

                    # First real character is not '[' — definitely not a signal
                    if stripped and stripped[0] != "[":
                        classified = True
                        yield buffer
                        continue

                    # Check for [SEARCH]
                    if SEARCH_SIGNAL in stripped:
                        logger.info(
                            f"[classify] LLM signal [SEARCH] for: "
                            f"'{user_message[:60]}...'"
                        )
                        await response.close()
                        yield SEARCH_SIGNAL
                        return

                    # Check for [TEMPLATE:id]
                    tmpl_match = re.search(r"\[TEMPLATE:([^\]]+)\]", stripped)
                    if tmpl_match:
                        template_id = tmpl_match.group(1).strip()
                        signal = f"[TEMPLATE:{template_id}]"
                        logger.info(
                            f"[classify] LLM signal {signal} for: "
                            f"'{user_message[:60]}...'"
                        )
                        await response.close()
                        yield signal
                        return

                    # Buffer too long to be any signal — treat as answer
                    if len(stripped) > max_signal_len + 2:
                        classified = True
                        yield buffer
                        continue
                else:
                    yield token
        except GeneratorExit:
            return
        except Exception as e:
            logger.error(f"[classify] Stream error: {e}")
            if buffer and not classified:
                yield buffer

        # Stream ended during buffering
        if not classified and buffer:
            stripped = buffer.strip()
            if SEARCH_SIGNAL in stripped:
                logger.info(
                    f"[classify] LLM signal [SEARCH] for: '{user_message[:60]}...'"
                )
                yield SEARCH_SIGNAL
                return

            tmpl_match = re.search(r"\[TEMPLATE:([^\]]+)\]", stripped)
            if tmpl_match:
                template_id = tmpl_match.group(1).strip()
                signal = f"[TEMPLATE:{template_id}]"
                logger.info(
                    f"[classify] LLM signal {signal} for: '{user_message[:60]}...'"
                )
                yield signal
                return

            yield buffer

    async def chat_stream(
        self,
        user_message: str,
        context: str | None = None,
        history: list[dict] | None = None,
    ) -> AsyncIterator[str]:
        """Stream chat response."""
        base_prompt = SYSTEM_PROMPT if context is not None else SYSTEM_PROMPT_GENERAL
        messages = [{"role": "system", "content": base_prompt}]
        mode_prompt = MODE_DOCS_PROMPT if context is not None else MODE_GENERAL_PROMPT
        messages.append({"role": "system", "content": mode_prompt})

        if history:
            tail = 6 if context is not None else 4
            messages.extend(history[-tail:])

        if context is not None:
            prompt = PROMPT_WITH_CONTEXT.format(context=context, question=user_message)
        else:
            prompt = user_message

        messages.append({"role": "user", "content": prompt})

        response = await self._client.chat.completions.create(
            model=self._model,
            messages=messages,
            max_tokens=self._max_tokens,
            temperature=self._temperature,
            stream=True,
        )

        async for chunk in response:
            if chunk.choices[0].delta.content:
                yield chunk.choices[0].delta.content

    async def generate_search_query(
        self,
        user_message: str,
        history: list[dict] | None = None,
    ) -> str:
        """Rewrite user query into a standalone search query."""
        if not history:
            return user_message

        messages: list[dict] = [
            {"role": "system", "content": SEARCH_QUERY_REWRITE_PROMPT},
        ]
        messages.extend(history[-4:])
        messages.append(
            {"role": "user", "content": f"Останнє питання користувача: {user_message}"}
        )

        try:
            response = await self._client.chat.completions.create(
                model=self._model,
                messages=messages,
                max_tokens=100,
                temperature=0.1,
            )
            rewritten = (response.choices[0].message.content or "").strip()
            if not rewritten:
                return user_message

            logger.info(
                f"[rewrite] Search query: '{user_message[:60]}...' -> '{rewritten[:120]}...'"
            )
            return rewritten
        except Exception as e:
            logger.warning(f"[rewrite] Failed, using original query: {e}")
            return user_message
