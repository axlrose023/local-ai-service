...
class ChatService:
    ...
    async def process_message(
        ...
        Flow:
            1. LLM classify_and_stream — either answers directly or signals
            2. If [SEARCH]: search DB -> second LLM call with context
            3. If [TEMPLATE:id]: pass template_id to presentation layer
            4. If direct answer: stream tokens from first call

        Yields:
            Tuples of (token, search_response, answer_mode, context_used).
            For template mode: answer_mode="template", context_used=template_id.
        """
        ...
        # Phase 1: LLM classifies and either answers or signals
        signal = None
        template_id = None
        ...
        async for token in self._llm.classify_and_stream(
            ...
            if token == _SEARCH_SIGNAL:
                ...
                continue
            if token.startswith(_TEMPLATE_PREFIX) and token.endswith("]"):
                signal = _TEMPLATE_PREFIX
                template_id = token[len(_TEMPLATE_PREFIX):-1].strip()
                continue
            if signal is not None:
                continue
            buffer_tokens.append(token)

        if signal == _TEMPLATE_PREFIX:
            # Template request — pass template_id to presentation layer
            yield ("", None, "template", template_id)
            return
        ...
        if signal == _SEARCH_SIGNAL:
            ...
            if not search_response.results:
                ...
                yield ("В документах УДО немає інформації з цього питання.", None, "docs", None)
                return
